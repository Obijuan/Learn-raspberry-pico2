...
#-- Las variables inicializadas son las que están en .data
#-- Sus valores están almacenados en la flash. Esta rutina copia
#-- los valores de la Flsh a la RAM
#-- Las direcciones se obtienen de etiquetas definidas en el linker 
#-- script
#---------------------------------------------------------------------
runtime_init_vars:

    #-- Leer las direcciones del linker script
    la t0, __data_flash_ini
    la t1, __data_ram_ini
    la t2, __data_ram_end

_init_vars_loop:
    #-- Comprobar si la direccion actual destino ha alcanzado el limite
    #-- Si es asi hemos terminado
    beq t1, t2, _init_vars_end
